...
// Thanks for Victor Laskin, and Mario Klingemann.
// using std library to support multi-threaded version
// by Kamenomi (https://github.com/Kamen-akio)
...
void stackblurJob(
    ...
    int core,             ///< current instThread number
  ...
  if (step == 1) {
    ...
    for (y = minY; y < maxY; y++) {
      sum_r = sum_g = sum_b = sum_a = sum_in_r = sum_in_g = sum_in_b =
          sum_in_a = sum_out_r = sum_out_g = sum_out_b = sum_out_a = 0;
...
class BlurTask {
 public:
  ...
  inline BlurTask(unsigned char* src, unsigned int w, unsigned int h,
    ...
    // instThread = thread(&BlurTask::run, this);
  }
...
void stackblur(
    ...
    unsigned int h,       ///< image height
    unsigned int radius,  ///< blur intensity (should be in 2..254 range)
    int cores = 1         ///< number of threads (1 - normal single instThread)
) {
  ...
  if (cores == 1) {
    ...
    BlurTask** workers = new BlurTask*[cores];
    for (int i = 0; i < cores; i++) {
      workers[i] =
          new BlurTask(src, w, h, radius, cores, i, 1, stack + div * 4 * i);
      workers[i]->Start();
    }

    for (int i = 0; i < cores; i++) workers[i]->Wait();
...
void Core::StackBlur(unsigned char* ptrBitmap, unsigned int width,
                     unsigned int height, unsigned int radius,
                     int threadCount = 1) {
  stackblur(ptrBitmap, width, height, radius, threadCount);
}
